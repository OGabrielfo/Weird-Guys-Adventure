...
public class FlyingChase : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        _controller = animator.GetComponent<BatController>();
    ...
    {
        if (animator.GetBool("ChasingPlayer"))
        {
            Vector2 direction = _player.transform.position - animator.transform.position;
            direction.Normalize();
            _rb.velocity = direction * animator.GetFloat("Velocity");
        }
        else
        {
